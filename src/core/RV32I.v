...
module rv32i_single_cycle #(
    ...
    // Temporary dummy control signals (Will be generated by FSM later)
    wire IRWrite = 1'b1; // Dummy: constantly update IR for now to keep single-cycle working
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            // We will wire these properly in the next steps, but they instantiate the hardware!
            // MDR <= mem_data;
            // A <= read_data1;
            // B <= read_data2;
            // ALUOut <= alu_result;
    ...
    wire [1:0] ALUSrcA_ctrl = 2'b01; // DUMMY: Hardcoded to A for now to prevent compiling errors
    ...
    wire [1:0] ALUSrcB_ctrl = 2'b10; // DUMMY: Hardcoded to Immediate for now
    ...
    wire [1:0] PCSource_ctrl = 2'b00;   // DUMMY: Hardcoded to alu_result for now
    wire PCWrite = 1'b1;                // DUMMY: Always write for now
